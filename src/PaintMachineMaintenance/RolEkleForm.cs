    ...
    public partial class RolEkleForm : Form
    {
        ...
        private void btnEkle_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrWhiteSpace(txtPersonelAd.Text) ||
                ...
                string.IsNullOrWhiteSpace(txtPersoneliletişim.Text))
            {
            ...
            try
            {
                using (var conn = DatabaseHelper.GetConnection())
                {
                    ...
                    string sql = @"INSERT INTO personel
                                   ...
                                   VALUES (@rolno, @ad, @soyad, @iletişim, @personelrolno)";
                    ...
                    using (var cmd = new NpgsqlCommand(sql, conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@iletişim", txtPersoneliletişim.Text.Trim());
                        cmd.Parameters.AddWithValue("@personelrolno", rolno); // personelrolno = rolno
                        cmd.ExecuteNonQuery();
                    }
                }

                MessageBox.Show("o Personel başarıyla eklendi!");
        ...
        private void btnSil_Click(object sender, EventArgs e)
        {
            ...
            string iletişim = dgvRoller.SelectedRows[0].Cells["personeliletisim"].Value.ToString();
            ...
            try
            {
                using (var conn = DatabaseHelper.GetConnection())
                {
                    ...
                    string sql = "DELETE FROM personel WHERE personeliletisim = @iletişim";

                    using (var cmd = new NpgsqlCommand(sql, conn))
                    {
                        cmd.Parameters.AddWithValue("@iletişim", iletişim);
                        cmd.ExecuteNonQuery();
                    }
                }

                MessageBox.Show("o Personel silindi!");
